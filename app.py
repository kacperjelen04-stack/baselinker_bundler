...
class BaseLinkerClient:
    ...
    def get_merged_features(self, product_ids: list[str]) -> tuple[dict[str, str], list[str]]:
        """Pobiera i scala cechy (text_fields.features) podanych produktów w jeden
        płaski słownik nazwa->wartość. Jeśli różne składniki mają tę samą nazwę
        cechy, ale RÓŻNE wartości (np. dwie różne pojemności baterii AA/AAA),
        wartości są łączone znakiem "|" bez spacji wokół niego (np.
        "2000mAh|1000mAh") - to format pola wielokrotnego wyboru w BaseLinkerze.
        Jeśli dane pole w BaseLinkerze jest w rzeczywistości polem
        jednowartościowym (np. czysto liczbowym), taki połączony zapis nie
        będzie poprawny - dlatego druga zwracana wartość to lista nazw cech,
        przy których wystąpił taki konflikt, żeby dało się to zalogować i
        w razie potrzeby poprawić ręcznie w panelu.
        Zwraca (scalone_cechy, lista_nazw_cech_z_konfliktem)."""
        self._ensure_product_data(product_ids)
        values_seen: dict[str, list[str]] = {}
        for pid in product_ids:
            for key, value in self._product_data_cache.get(pid, {}).get("features", {}).items():
                bucket = values_seen.setdefault(key, [])
...
class BundleImportJob:
    ...
    def run(self, rows: list[dict]) -> None:
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            try:
                features, feature_conflicts = self.client.get_merged_features(component_ids)
                if feature_conflicts:
                    self._emit(
                        "warning",
                        f"Wiersz {idx}: składniki mają różne wartości cechy "
                        f"{', '.join(feature_conflicts)} - połączono je znakiem \"|\" "
                        "(format pola wielokrotnego wyboru). Jeśli w BaseLinkerze to pole "
                        "przyjmuje tylko jedną wartość, popraw ją ręcznie w panelu.",
                        row=idx,
                    )
            except BaseLinkerAPIError as exc:
                features = {}
...
app = Flask(__name__)
app.config["MAX_CONTENT_LENGTH"] = 300 * 1024 * 1024  # 300 MB - pełne oferty XML bywają bardzo duże
...
def handle_too_large(_exc):
    ...
    return (
        jsonify({"ok": False, "error": f"Plik jest za duży (limit: {limit_mb} MB)."}),
        413,
    )
...
JOBS: dict[str, dict] = {}
XML_UPLOADS: dict[str, list] = {}  # upload_id -> lista sparsowanych rekordów (xml.etree.Element)
...
@app.route("/api/xml/inspect", methods=["POST"])
def api_xml_inspect():
    uploaded = request.files.getlist("files")
    if not uploaded:
        return jsonify({"ok": False, "error": "Wgraj co najmniej jeden plik XML."}), 400
    if len(uploaded) > 2:
        return jsonify({"ok": False, "error": "Można wgrać maksymalnie dwa pliki XML."}), 400

    file_bytes = []
    for f in uploaded:
        data = f.read()
        if not data:
            return jsonify({"ok": False, "error": f"Plik '{f.filename}' jest pusty."}), 400
        file_bytes.append(data)

    try:
        result = xml_tools.inspect_xml_files(file_bytes)
    except xml_tools.XmlParseError as exc:
        return jsonify({"ok": False, "error": exc.message}), 400
    except Exception as exc:  # nieoczekiwany błąd parsowania - nie wywalamy 500 bez komunikatu
        return jsonify({"ok": False, "error": f"Nie udało się przetworzyć pliku: {exc}"}), 400

    upload_id = str(uuid.uuid4())
    with STATE_LOCK:
        XML_UPLOADS[upload_id] = result["_records"]
...
def api_xml_generate():
    payload = request.get_json(force=True, silent=True) or {}
    upload_id = payload.get("upload_id")
    mapping = payload.get("mapping") or {}

    if not upload_id:
        return jsonify({"ok": False, "error": "Brak upload_id - wczytaj plik ponownie."}), 400
    if not mapping:
        return jsonify({"ok": False, "error": "Przypisz przynajmniej jedno pole do kolumny CSV."}), 400

    with STATE_LOCK:
        records = XML_UPLOADS.get(upload_id)
    if records is None:
        return jsonify({"ok": False, "error": "Nie znaleziono wgranego pliku - wczytaj go ponownie."}), 404

    csv_text = xml_tools.generate_csv(records, mapping)
    return Response(
        csv_text,
        mimetype="text/csv",
        headers={"Content-Disposition": 'attachment; filename="konwersja.csv"'},
    )
